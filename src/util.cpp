...
const char * const BITCOIN_CONF_FILENAME = "hashrentalcoin.conf";
const char * const BITCOIN_PID_FILENAME = "hashrentalcoind.pid";
 ...
 * by well-meaning hashrentalcoin adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "hashrentalcoin" is a composite category enabling all HashRentalCoin-related debug output
            if(ptrCategory->count(string("hashrentalcoin"))) {
...
#else
    const char* pszModule = "hashrentalcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HashRentalCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HashRentalCoinCore
    // Mac: ~/Library/Application Support/HashRentalCoinCore
    // Unix: ~/.hashrentalcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HashRentalCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HashRentalCoinCore";
#else
    // Unix
    return pathRet / ".hashrentalcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty hashrentalcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override hashrentalcoin.conf
