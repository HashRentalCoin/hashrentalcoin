...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all HashRentalCoin related functionality
...
/*
    ...
    allow endless transaction that would bloat HashRentalCoin and make it unusable. To
...
void CPrivateSendServer::CheckTimeout(CConnman& connman)
{
    ...
    int nLagTime = fMasterNode ? 0 : 2000; // if we're the client, give the server a few extra seconds before resetting.
...
{
    if(fLiteMode) return; // disable all HashRentalCoin specific functionality
    ...
    RenameThread("hashrentalcoin-ps-server");
